...
Partial Class _Default
    ...
    Private Sub getrcdforedit()
        Try
            qry = "select * from ams.ams_desgmaster where desgid='" & Request.QueryString("empid") & "'"
            dt = clas.getdata(qry, "QR")
            If dt.Rows.Count > 0 Then
                TextBox1.Text = Convert.ToString(dt.Rows(0)("desgname"))
                TextBox10.Text = Convert.ToString(dt.Rows(0)("desgdesc"))
                DropDownList2.Items.FindByValue(Convert.ToString(dt.Rows(0)("desgsts"))).Selected = True
    ...
    Private Function Check_DesignationStatus() As Boolean
        Dim query As String = ""
        If Button1.Text = "Update" Then
            query = "select * from ams.ams_desgmaster where LOWER(desgname)='" & LCase(TextBox1.Text.Trim) & "' and desgid<>'" & Request.QueryString("empid") & "'"
        Else
            query = "select * from ams.ams_desgmaster where LOWER(desgname)='" & LCase(TextBox1.Text.Trim) & "'"
        End If
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click, ImageButton1.Click
        ...
        Check_Disposition = Check_DesignationStatus()
        If Check_Disposition = False Then
            RadWindowManager1.RadAlert("<ul><li><div style=color:green;> Entered Designation is already in existance. </div></li></ul>", 300, 150, "Validation Success", Nothing)
            Exit Sub
        End If
        ...
        If flag = True Then
            ...
            If Button1.Text = "Update" Then
                Button1.Text = "Submit"
                RadWindowManager1.RadAlert("<ul><li><div style=color:green;> Designation information updated successfully. </div></li></ul>", 300, 150, "Validation Success", Nothing)
            Else
                Button1.Text = "Submit"
                RadWindowManager1.RadAlert("<ul><li><div style=color:green;> Designation information saved successfully. </div></li></ul>", 300, 150, "Validation Success", Nothing)
            End If
    ...
    Public Function InsertCommand() As Boolean
        Try
            ...
            Dim transid As Int32 = clas.getMaxID("select ams.ams_desgmaster_seq.nextval as id from dual")
        ...
        str.Append("insert into ams.ams_desgmaster(desgid,")
    ...
    Public Function updateCommand() As Boolean
        Try
            ...
            'Dim str2 As String = "insert into tt.ukdata(transid, InDocRecdDt,InDocRecdBy, InDocStat) values(2, to_date(@InDocRecdDt,'dd-MON-yyyy'), @InDocRecdBy, @InDocStat)"
            Dim str As StringBuilder = New StringBuilder
            str.Append("update ams.ams_desgmaster set ")


            If TextBox1.Text <> "" Then
                str.Append("desgname='" & TextBox1.Text & "',")
            End If


            If TextBox10.Text <> "" Then

                str.Append("desgdesc='" & TextBox10.Text & "',")
            End If
            If DropDownList2.SelectedValue <> "" Then
                str.Append("desgsts='" & DropDownList2.SelectedValue & "',")
            End If
            Dim temp As String = str.ToString
            str.Clear()
            str.Append(temp.Substring(0, temp.Length - 1))
            str.Append(" where desgid='" & Request.QueryString("empid") & "'")
