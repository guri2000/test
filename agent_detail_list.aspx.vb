...
Partial Class Default2
    ...
    Private Sub getBlank()
        qry = "select * from ams.ams_agentData where 1=0"
    ...
    Private Sub getmastres()
        Try
            'qry = "select agntname as Agent_Name, agntdesg as designation,agnteml as Agent_Email, agnttelno as Agent_Mobile,lgnnam as Login_Name,decode(lgnsts,'A','Active','In-Active') Account_status,decode(lgnroll,'S','Adminstrator','Agent') Account_Acess, to_char(lastlogin,'dd-Mon-yy HH24:mi:ss') as last_login, agntid from ams.ams_agents a, ams.ams_usermaster b where a.agntid=b.lgnagntid"
            ' Above Query commented due to remove time from Login
            qry = "select * from ams.ams_agentData"

            'qry = "select agntname as Agent_Name, agntdesg as designation,agnteml as Agent_Email, agnttelno as Agent_Mobile,lgnnam as Login_Name,decode(lgnsts,'A','Active','In-Active') Account_status,decode(lgnroll,'S','Adminstrator','Agent') Account_Access, to_char(lastlogin,'dd-Mon-yyyy') as last_login,'Password: '||b.lgnpwd as pwd, agntid from ams.ams_agents a, ams.ams_usermaster b where a.agntid=b.lgnagntid"

            ' qry = "select agntname as Agent_Name, agntdesg as designation,agnteml as Agent_Email, agnttelno as Agent_Mobile,lgnnam as Login_Name,decode(lgnsts,'A','Active','In-Active') Account_status,decode(lgnroll,'S','Adminstrator','Agent') Account_Acess, to_char(lastlogin,'dd-Mon-yy HH24:mi:ss') as last_login, agntid from ams.ams_agents a, ams.ams_usermaster b where a.agntid=b.lgnagntid and lgnroll not in ('S')"
    ...
    Protected Sub RadGrid1_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles RadGrid1.ItemDataBound
        ...
        If (TypeOf e.Item Is GridDataItem) Then
            ...
            Dim btnedit As ImageButton = DirectCast(item.FindControl("btnedit"), ImageButton)
			  Dim tipimg As HtmlImage = DirectCast(item.FindControl("tipimg"), HtmlImage)
            ...
            btnedit.CommandArgument = item.GetDataKeyValue("agntid").ToString()
    ...
    Protected Sub RadGrid1_ItemCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles RadGrid1.ItemCommand
        ...
        If e.CommandName = "editform" Then
            edtti.InnerText = "Edit Agent Information"
            Ifrmfollowup.Attributes.Add("src", "agent_det.aspx?empid=" & e.CommandArgument.ToString() & "&mode=edit")
            'ClientScript.RegisterStartupScript(Me.GetType(), "none", "<script>$('#addnewfollowup').modal('show');</script>", True)
            ClientScript.RegisterStartupScript(Me.GetType(), "alert", "ShowPopup();", True)
            'ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "openModal();", True)
